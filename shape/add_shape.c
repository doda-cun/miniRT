...
/*   Updated: 2026/04/14 13:28:27 by doda-cun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_cylinder_to_set(t_scene *scene, int i)
{
	...
	scene->cylinder[i].radius = scene->cylinder[i].radius;
	add_shapeset(&scene->shapeset, (t_shape *)&scene->cylinder[i]);
	scene->cylinder[i].shape.single_intersection = sphere_single_intersect; // place holder
}
