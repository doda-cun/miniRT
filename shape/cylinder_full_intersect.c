...
/*   Updated: 2026/04/20 18:33:33 by doda-cun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							t_point cap_center)
{
	...
	d_dot_n = dot_product(intersection->ray.direction, cylinder->axis);
	if (d_dot_n == 0.0f)
		return (0);
...
Strip out everything parallel to the cylinders axis
from both the ray direction and the origin offset.
What's left (v_perp, dp_perp) describes the ray
as seen from directly above the cylinder —
where it looks like a simple circle problem.

2. Solve the quadratic for the curved surface.
Same At² + Bt + C = 0 structure as the sphere.
The discriminant tells you if the ray misses entirely (< 0),
...
You project the hit point back onto the axis
 and check it falls between 0 and height.
...
followed by checking the hit point falls within the disc radius.
This closes off the top and bottom.

5. The closest valid t wins.
Because every hit function checks t >= intersection->length before accepting,
and the shapeset loop runs all shapes,
whichever shape produces the smallest t is what ends up coloring the pixel —
exactly the same mechanism as sphere vs plane.

if (d_dot_n == 0.0f) //ray runs parallel to cap of cylinder, no hit
if (dot_product(hit_to_cap, hit_to_cap) > squared(cylinder->radius))
